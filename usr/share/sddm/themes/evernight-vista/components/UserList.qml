...
ListView {
    ...
    readonly property int userItemWidth: Kirigami.Units.gridUnit * 3
    readonly property int userItemHeight: Kirigami.Units.gridUnit * 3
    ...
    delegate: UserDelegate {
        ...
        name: {
            ...
            if (!model.session) {
                return i18ndc("plasma-desktop-sddm-theme", "Nobody logged in on that session", "Unused")
            }


            let location = undefined
            if (model.isTty) {
                location = i18ndc("plasma-desktop-sddm-theme", "User logged in on console number", "TTY %1", model.vtNumber)
            } else if (model.displayNumber) {
                location = i18ndc("plasma-desktop-sddm-theme", "User logged in on console (X display number)", "on TTY %1 (Display %2)", model.vtNumber, model.displayNumber)
            }

            if (location !== undefined) {
                return i18ndc("plasma-desktop-sddm-theme", "Username (location)", "%1 (%2)", displayName, location)
            }
